...
Используемые технологии:
	...
	PS PDO я не использую (во первых мне нужно рабочее кеширование Mysql, во вторых все запросы к БД мне важно видеть на чистом SQL), однако защита от SQL инъекций имеется
	PS Memcahe и тп я не использую (там где это эксктренно нужно, например при работе со справочниками , все даныне я привык помещать в ассоциативный массив и брать из него)
...
После клонирования репозитория необходимо
	...
	PS В случае если они добавились в сотлеживание их можно удалить командой:	git update-index --skip-worktree path/to/file . Так можно и свои правки в преокт вносить и обновлять
