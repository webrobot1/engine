...
Если проект ведется от master ветки (те без Fork),то для постоянного обновления без перезаписи ваших настроек нужно (в тч на удаленном сервере и на рабочей машине):

	выполнить команду "git update-index --skip-worktree .gitmodules" (что бы не предлагать отправлять Push запрос в master на добавленные <дополнительные приложения>)
