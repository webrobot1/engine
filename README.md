...
Общеизвестный прицип:
	...
	L. - да, пример - фаилы выше и  https://github.com/webrobot1/app-map/tree/master/model/Tiled (сохранение карт Tiled из Xml в объекты PHP , загрузка - сохранения из Mysql, формирование изображения на GD)
...
После клонирования репозитория необходимо
	...
	выполнить команду "git update-index --skip-worktree .gitmodules" (что бы создавать свои субмодули и не предлагать сделать Pull запрос в родителя)
	выполнить команду "git update-index --skip-worktree composer.json" (что бы добавлять свои зависимости и не предлагать сделать Pull запрос в родителя)
	пареключить рабочую директорию в командной строке на app/<приложение> и выполнить "git update-index --skip-worktree cfg/" (что бы менять настройки БД и не предлагать выгружать в GIT более)	
