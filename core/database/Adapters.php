...
{	
	...
	function __construct(array $config)
	{
		...
		{
			if(exec('ifconfig -a|grep inet|grep -v 127.0.0.1|grep -v inet6|awk \'{print $2}\'|tr -d "addr:"', $ip))
				static::$remote_ip = $ip[0];
			
			static::$host = gethostname();
			static::$local_ip = gethostbyname(static::$host);
		}	
		if($config['host']==static::$local_ip || $config['host']==$_SERVER['SERVER_ADDR'] || (!empty(static::$remote_ip) && $config['host']==static::$remote_ip)){
